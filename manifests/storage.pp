...
define duplicacy::storage (
        ...
        #
        $error_prefix = "Repo/Storage: ${repo_id}/${storage_name},"
        ...
        if !empty($encryption) {
                ...
                if 'password' in $encryption {
                        ...
                        $repo_init_env = $repo_init_env +
                                "${env_prefix}_PASSWORD=${password}"
                        $repo_init_command="${repo_init_command} -e"
                } else {
                        fail("${error_prefix} Password mandatory when encryption is enabled!")
                }

                if 'iterations' in $encryption {
                        $iterations = $encryption['iterations']
                        $repo_init_command =
                                "${repo_init_command} -iterations ${iterations}"
                }
        }
        ...
        if !empty($chunk_parameters) {
                ...
                $repo_init_command =
                        "${repo_init_command} -chunk-size ${chunk_size}"
                ...
                $repo_init_command =
                        "${repo_init_command} -max-chunk-size ${chunk_size_max}"
                ...
                $repo_init_command =
                        "${repo_init_command} -max-chunk-size ${chunk_size_min}"
        }

        # Process storage URL
        if !empty($target) {
                fail("${error_prefix} Target is mandatory!")
        }
        ...
        if 'url' in $target {
                ...
                fail("${error_prefix} Target URL is mandatory!")
        }
        ...
        case $storage_url {
                /^b2:/: {
                        if ! ('b2_account_id' in $target) or
                                ...
                                fail("${error_prefix} b2_account_id and b2_application_key are mandatory for ${storage_url}")
                        }
                        ...
                        $repo_init_command =
                                "${repo_init_command} ${repo_id} ${storage_url}"
                }
                default: {
                        fail("${error_prefix} Unrecognized url: ${storage_url}")
                }
        }
